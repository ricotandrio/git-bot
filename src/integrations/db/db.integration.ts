  ...
  async execute(action: DbAction, payload: any) {
    switch (action) {
      case 'add_guild_repository':
        return addGuildRepository(this.getClient().getDb(), payload);
      case 'remove_guild_repository':
        return removeGuildRepository(this.getClient().getDb(), payload);
      case 'get_guild_repositories':
        return getGuildRepositories(this.getClient().getDb(), payload);
      case 'get_user_mapping':
        return getUserMapping(this.getClient().getDb(), payload);
      case 'set_user_mapping':
        return setUserMapping(this.getClient().getDb(), payload);
      case 'delete_user_mapping':
        return deleteUserMapping(this.getClient().getDb(), payload);
      default:
